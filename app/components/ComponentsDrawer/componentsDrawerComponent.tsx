...
const ComponentsDrawerComponent: React.FC<Props> = ({ component }) => {
  ...
  return (
      <div className="card my-2">
        <div className="card-body d-flex justify-content-between" onClick={() => addEditorContent(component.content)}>
          <p>{component.name}</p>
          { component.description ?
            <p onClick={(evt) => {
              evt.stopPropagation()
              setSelectedComponent(component)
            }} data-bs-toggle="modal" data-bs-target={`#componentDescriptionModal`} ><InfoIcon /></p>
            : ""
          }  
        </div>
      </div>
